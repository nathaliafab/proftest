...
export class DatabaseQuestionBank {
  ...
  async modifyQuestion(id: string, newDescription: string, newAnswers: any[]): Promise<void> {
    const db = await getDb();
  ...
  async getQuestionByDescription(description: string): Promise<Question | undefined> {
    const db = await getDb();
    const [row] = await db.select().from(questionsTable).where(eq(questionsTable.description, description));
