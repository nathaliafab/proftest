export class CorrectionService {
  async grade(answersCsv: string, responsesCsv: string, mode: 'rigorous' | 'proportional'): Promise<any[]> {
    ...
    for (const data of answersData) {
      ...
      const testNoIdx = headers.findIndex((h: string) => h.trim().toLowerCase().includes('number') || h.trim().toLowerCase().includes('prova'));
      
      if (testNoIdx !== -1) {
        const testNo = values[testNoIdx]?.trim();
        const expectedForTest = values.filter((_: any, i: number) => i !== testNoIdx).map((v: string) => {
    ...
    for (const data of responsesData) {
      ...
      const studentIdx = headers.findIndex((h: string) => h.trim().toLowerCase().includes('aluno') || h.trim().toLowerCase().includes('name') || h.trim().toLowerCase().includes('student'));
      const testNoIdx = headers.findIndex((h: string) => h.trim().toLowerCase().includes('prova') || h.trim().toLowerCase().includes('test'));

      if (studentIdx === -1 || testNoIdx === -1 || !values[studentIdx]) continue;
      ...
      let studentAnswers = values
        .filter((_: any, i: number) => i !== studentIdx && i !== testNoIdx)
        .map((v: string) => (v || "").replace(/^"|"$/g, '').trim().replace(/ /g, ''));
      ...
      if (studentAnswers.length === 1 && expectedAnswers && expectedAnswers.length > 1 && studentAnswers[0].length > 1) {
