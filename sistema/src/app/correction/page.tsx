...
import { Download, UploadCloud, Users, Eye, X } from 'lucide-react';
...
export default function CorrectionPage() {
  ...
  const handleProcess = async () => {
    ...
    if (!answersCsv || !responsesCsv) return alert('Envie ambos os arquivos CSV.');
  ...
  const avgGrade = report ? (report.reduce((acc, r) => acc + r.nota_final, 0) / report.length).toFixed(1) : '0';
  const successRate = report 
    ? Math.round((report.filter(r => r.acertos >= ((r.total_questoes || 10) * 0.7)).length / report.length) * 100) 
    : 0;
      ...
      <main className={styles.mainContent}>
        ...
        <section className={styles.topSection}>
          <div className={styles.importPanel}>
            ...
            <div>
              <label className={styles.sectionTitle}>PROVA</label>
              <select 
            ...
            <div className={styles.dropzonesGrid}>
              <div>
                <label className={styles.sectionTitle}>GABARITO OFICIAL (KEY CSV)</label>
                <div 
                  className={styles.dropzone} 
                  onClick={() => document.getElementById('answersUpload')?.click()}
                  ...
                  <div className={styles.dropzoneText}>
                    {answersCsv ? (
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
                        {answersCsv.name}
                        <button 
                          onClick={(e) => { e.stopPropagation(); setAnswersCsv(null); (document.getElementById('answersUpload') as HTMLInputElement).value = ''; }}
                    ...
                    ) : <>Arraste o arquivo ou <span className={styles.dropzoneLink}>clique aqui</span></>}
                  </div>
              ...
              <div>
                <label className={styles.sectionTitle}>RESPOSTAS DOS ALUNOS (ANSWERS CSV)</label>
                <div 
                  className={styles.dropzone} 
                  onClick={() => document.getElementById('responsesUpload')?.click()}
                  onDragOver={handleDragOver}
                ...
                >
                  ...
                  <div className={styles.dropzoneText}>
                    {responsesCsv ? (
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
                        {responsesCsv.name}
                        <button 
                          onClick={(e) => { e.stopPropagation(); setResponsesCsv(null); (document.getElementById('responsesUpload') as HTMLInputElement).value = ''; }}
                          style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'flex', color: '#EF4444' }}
                          title="Remover arquivo"
                        >
                          <X size={18} />
                  ...
                  <div className={styles.dropzoneSubText}>Formato: nome_aluno, tipo_prova, resposta1, resposta2, ...</div>
                  <input 
                    id="responsesUpload" 
                    type="file" 
                    accept=".csv" 
                    style={{ display: 'none' }} 
                    onChange={(e) => setResponsesCsv(e.target.files?.[0] || null)} 
                  />
                </div>
              </div>
            </div>
            ...
            <div className={styles.rigorOptions}>
              ...
              >
                ...
                <div className={styles.rigorOptionContent}>
                  ...
                  <div className={styles.rigorOptionDesc}>Critério "Tudo ou Nada". Ideal para provas objetivas de múltipla escolha simples.</div>
                </div>
              </div>
              ...
              >
                ...
                <div className={styles.rigorOptionContent}>
                  ...
                  <div className={styles.rigorOptionDesc}>Pontuação parcial para questões complexas ou somatórias.</div>
                </div>
              </div>
            </div>
          ...
          <section className={styles.reportSection}>
            <div className={styles.reportHeader}>
              <div>
                ...
                <div className={styles.reportSub}>Visualização rápida dos últimos processamentos realizados.</div>
              </div>
            ...
            <div className={styles.tableContainer}>
              <table className={styles.table}>
                ...
                <tbody>
                  {report.map((r, i) => {
                    // Extract initials
                    const initials = r.aluno.split(' ').map((n: string) => n[0]).join('').substring(0, 2).toUpperCase();
                    // Define values
                    const acertos = r.acertos;
                    const totalQ = r.total_questoes || 10;
                    const notaFinal = r.nota_final;
                    const passThreshold = totalQ * 0.7; // 70% of total questions
                    const statusVal = acertos >= passThreshold ? 'APROVADO' : 'RECUPERAÇÃO';
                    const statusClass = acertos >= passThreshold ? styles.statusPass : styles.statusWarning;
                    ...
                    return (
                      <tr key={i}>
                        ...
                        <td>{acertos}/{totalQ}</td>
                        <td className={acertos >= passThreshold ? styles.gradeHigh : styles.gradeBorderline}>
                          {notaFinal.toFixed(1)}
                        </td>
                        <td>
                          <span className={`${styles.statusBadge} ${statusClass}`}>{statusVal}</span>
                        </td>
                      </tr>
                    );
                  })}
            ...
            <div className={styles.statsFooter}>
              <div className={styles.statsGroup}>
                ...
                <div className={styles.statItem}>
                  ...
                  <span className={`${styles.statValue} ${styles.statValueSuccess}`}>{successRate}%</span>
                </div>
              </div>
              <div className={styles.statsActions}>
                <button className={styles.btnCancel} onClick={() => setReport(null)}>Cancelar Processo</button>
                <button className={styles.btnPublish}>PUBLICAR NOTAS</button>
              </div>
            </div>
          </section>
        )}
