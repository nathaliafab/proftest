...
export default function ManageTestsPage() {
	...
	const handleToggleQuestion = (
		id: string,
		style: IdentifierStyle = "letters",
	) => {
		const exists = selectedQuestions.find((q) => q.questionId === id);
		if (exists) {
			setSelectedQuestions(
				selectedQuestions.filter((q) => q.questionId !== id),
			);
		} else {
			setSelectedQuestions([
				...selectedQuestions,
				{ questionId: id, identifierStyle: style },
			]);
		}
	};
	...
	const handleChangeQuestionStyle = (id: string, style: IdentifierStyle) => {
		setSelectedQuestions(
			selectedQuestions.map((q) =>
				q.questionId === id ? { ...q, identifierStyle: style } : q,
			),
		);
	};
		...
		<div className={styles.container}>
			<main className={styles.mainContent}>
				<header className={styles.header}>
					...
					<button className={styles.btnPrimary} onClick={handleCancelEdit}>
						Nova Prova
					</button>
				</header>
					...
					<div className={styles.leftColumn}>
						...
						<div className={styles.altsHeader}>
							<div className={styles.sectionTitle}>
								SELECIONAR QUESTÕES ({selectedQuestions.length} inclusas)
							</div>
							<span className={styles.altsHint}>
								Selecione as questões para a prova no checkbox lateral
							</span>
						</div>
												...
												{isSelected && (
													...
													>
														...
														<select
															...
															onChange={(e) =>
																handleChangeQuestionStyle(
																	q.id,
																	e.target.value as IdentifierStyle,
																)
															}
															style={{
																padding: "0.4rem",
						...
						<div className={styles.actions}>
							...
							>
								{editingId ? "Salvar Alterações" : "Salvar Nova Prova"}
							</button>
						</div>
					</div>
				</section>
					...
					<div className={styles.bankHeader}>
						<div>
							...
							<p className={styles.bankSub}>
								Provas recentemente editadas no projeto
							</p>
						</div>
						...
						<div className={styles.modalContent}>
							...
							<div className={styles.modalInputGroup}>
								...
								<input
									...
									value={pdfAmount}
									onChange={(e) =>
										setPdfAmount(parseInt(e.target.value, 10) || 1)
									}
								/>
							</div>
							...
							<div className={styles.actions} style={{ marginTop: "2rem" }}>
								<button className={styles.btnCancel} onClick={closePdfModal}>
									Cancelar
								</button>
								<button
									className={styles.btnPrimary}
