...
export async function GET(
  ...
  try {
    ...
    if (isNaN(amount) || amount < 1 || amount > 100) {
      return NextResponse.json({ error: 'A quantidade deve ser um número entre 1 e 100' }, { status: 400 });
    }
    ...
    if (validQuestions.length === 0) {
       return NextResponse.json({ error: 'Não há questões válidas associadas a esta prova' }, { status: 400 });
    }
    ...
    return new NextResponse(buffer as unknown as BodyInit, {
     ...
     return NextResponse.json({ error: 'Falha ao gerar as provas' }, { status: 500 });
  }
}
