	...
	try {
		...
		if (!testId || typeof testId !== "string" || !testId.trim()) {
			return NextResponse.json(
				{ error: "O ID da prova é obrigatório" },
				{ status: 400 },
			);
		}
		...
		if (!answersFile || typeof answersFile.text !== "function") {
			return NextResponse.json(
				{ error: "O arquivo de gabarito é inválido ou ausente" },
				{ status: 400 },
			);
		}
		...
		if (!responsesFile || typeof responsesFile.text !== "function") {
			return NextResponse.json(
				{ error: "O arquivo de respostas é inválido ou ausente" },
				{ status: 400 },
			);
		}
		...
		if (!mode || (mode !== "rigorous" && mode !== "proportional")) {
			return NextResponse.json(
				{ error: 'O modo de correção deve ser "rigorous" ou "proportional"' },
				{ status: 400 },
			);
		}
		...
		if (!answersText.trim()) {
			return NextResponse.json(
				{ error: "O arquivo de gabarito está vazio" },
				{ status: 400 },
			);
		}
		...
		if (!responsesText.trim()) {
			return NextResponse.json(
				{ error: "O arquivo de respostas está vazio" },
				{ status: 400 },
			);
		}
		...
		if (!currentReport) {
			return NextResponse.json(
				{ error: "Falha ao processar e gerar o relatório" },
				{ status: 500 },
			);
		}

		return NextResponse.json(
			{ success: true, testId, report: currentReport },
			{ status: 200 },
		);
	} catch (error) {
		...
		return NextResponse.json(
			{
				error: "Falha ao corrigir as provas, verifique o formato dos arquivos",
			},
			{ status: 500 },
		);
	}
